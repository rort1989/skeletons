...
for b = 1:20
    ...
    joint_locs_A_name = strcat('joint_locs_',num2str(a));
    joint_locs_B_name = strcat('joint_locs_',num2str(b));
    
    evalc(strcat('action_id_A =',action_id_A_name));
    evalc(strcat('action_id_B =',action_id_B_name));
    evalc(strcat('joint_locs_A =',joint_locs_A_name));
    evalc(strcat('joint_locs_B =',joint_locs_B_name));
    
    joint_locs = cell(1);
    
    for i = 1:size(action_id_A,1)
        if action_id_A(i,1) == b
            joint_locs = vertcat(joint_locs,joint_locs_A(i));
        end
        
        if size(joint_locs,1) == 16
            joint_locs = joint_locs(2:end,1);
            evalc(strcat('locs_',actions{b},sections{s},'= joint_locs'));
            save(strcat('Locations_',actions{b},sections{s},'.mat'),strcat('locs_',actions{b},sections{s}));
            s = s + 1;
            clear joint_locs
            joint_locs = cell(1);
        end
    ...
    if ~isequal(action_id_B,action_id_A) 
        for i = 1:size(action_id_B,1)
            if action_id_B(i,1) == b
                joint_locs = vertcat(joint_locs,joint_locs_B(i));
            end
