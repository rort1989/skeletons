...
for b = 1:20
    ...
    load (strcat('JointVelocities_',num2str(a,'%02d'),'_of_20'))
    load (strcat('JointVelocities_',num2str(b,'%02d'),'_of_20'))
   
    action_id_A_name = strcat('action_id_',num2str(a));
    action_id_B_name = strcat('action_id_',num2str(b));
    joint_vels_A_name = strcat('joint_vels_',num2str(a));
    joint_vels_B_name = strcat('joint_vels_',num2str(b));
    
    evalc(strcat('action_id_A =',action_id_A_name));
    evalc(strcat('action_id_B =',action_id_B_name));
    evalc(strcat('joint_vels_A =',joint_vels_A_name));
    evalc(strcat('joint_vels_B =',joint_vels_B_name));
    
    joint_vels = cell(1);
    
    for i = 1:size(action_id_A,1)
        if action_id_A(i,1) == b
            joint_vels = vertcat(joint_vels,joint_vels_A(i));
        end
        
        if size(joint_vels,1) == 16
            joint_vels = joint_vels(2:end,1);
            evalc(strcat('vels_',actions{b},sections{s},'= joint_vels'));
            save(strcat('Velocities_',actions{b},sections{s},'.mat'),strcat('vels_',actions{b},sections{s}));
            s = s + 1;
            clear joint_vels
            joint_vels = cell(1);
        end
    ...
    if ~isequal(action_id_B,action_id_A) 
        for i = 1:size(action_id_B,1)
            if action_id_B(i,1) == b
                joint_vels = vertcat(joint_vels,joint_vels_B(i));
            end
